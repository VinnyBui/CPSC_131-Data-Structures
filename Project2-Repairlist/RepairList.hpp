...
void RepairList::prev()
{
	if (_nowServicing != _dailyList.begin())
		...
		std::cout << "Begin of the list reached\n";
}
...
{
	std::list<Repair>::iterator _itr = _nowServicing;  //Created another pointer that can access the current iterator and be changed
	if (_itr == _dailyList.end())     //if its already at the end
		addToList(newRequest);
	else {
		_itr++;
		_dailyList.insert(_itr, newRequest);
	}
}